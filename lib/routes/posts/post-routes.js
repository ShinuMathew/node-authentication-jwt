...
class PostRoutes {
    ...
    async updatePost(req, res) {
        try {
            let result = await this.postService.updatePost(req)
            this.responseHandler.replySuccess(res, result, 202)
        } catch (err) {
            this.responseHandler.replyError(res, err)
        }
...
function registerPostRoutes() {
    ...
    router.put('/:id', [this.postRoutes.postMiddleware.updateUserSchema, this.postRoutes.postMiddleware.updateUserParam], (req, res) => this.postRoutes.updatePost(req, res))
