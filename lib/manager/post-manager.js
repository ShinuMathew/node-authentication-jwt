...
class PostManager {
    ...
    async updatePost(id, updatedData, user) {
        try {
            let result = await this.postDataAccessor.updatePost(id, updatedData, user.username);
            if(!result.acknowledged)
                throw Errors.UnexpectedErrorOccurred
            ...
            logger.error(`Following error occured when updating user data to mongodb :\n${JSON.stringify(err)}`)
            throw err
        }
    }
}
